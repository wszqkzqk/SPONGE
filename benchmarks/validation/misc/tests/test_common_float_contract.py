...
PROBE_SOURCE = Path(__file__).with_name("common_float_probe.cpp")
...
def _compiler_command():
    ...
    if compiler is None:
        pytest.skip("a C++17 compiler is required for the float-contract probe")
    return [compiler]
...
def _dependency_include():
    ...
    for candidate in candidates:
        if (candidate / "omp.h").is_file() and (candidate / "fftw3.h").is_file():
            return candidate
    pytest.skip("OpenMP and FFTW headers are required for the float-contract probe")


def test_ieee_float_memory_contract_survives_fast_math(tmp_path):
    executable = tmp_path / "common_float_probe"
    ...
    assert compile_result.returncode == 0, (
        f"failed to compile float-contract probe\nstdout:\n"
