...
static __device__ __forceinline__ void Dihedral_Fail_Invalid_Geometry(
    ...
    int* invalid_geometry_term, bool accumulator_failure = false)
{
    ...
    {
        printf(
            "Fatal SPONGE proper dihedral error: global term %d (local atoms "
            "%d %d %d %d) would produce a non-finite force/energy/virial "
            "accumulator.\n",
    ...
    else
    {
        printf(
            "Fatal SPONGE proper dihedral error: global term %d (local atoms "
            "%d %d %d %d) has undefined, non-finite, or unrepresentable "
            "torsion geometry/energy/force/virial.\n",
            global_term, atom_i, atom_j, atom_k, atom_l);
    }
#if defined(USE_CUDA)
    asm volatile("trap;");
...
#endif
#else
    // -1 means no failure.  A term index is at most INT_MAX - 1, so this
    // encoding remains representable even for the largest valid term table.
...
static __global__ void Dihedral_Force_With_Atom_Energy_And_Virial_Device(
    ...
    const float* gamc, const float* gams, const float* pn,
    const int* global_index, VECTOR* frc, int need_atom_energy, float* ene,
...
#pragma omp parallel for
...
#endif
    {
        ...
        const double temp_pn = static_cast<double>(pn[dihedral_i]);
        const double temp_gamc = static_cast<double>(gamc[dihedral_i]);
        const double temp_gams = static_cast<double>(gams[dihedral_i]);

        TORSION_GEOMETRY geometry;
        if (!Compute_Torsion_Geometry(crd[atom_i], crd[atom_j], crd[atom_k],
                                      crd[atom_l], cell, rcell, &geometry))
        {
            Dihedral_Fail_Invalid_Geometry(global_term, atom_i, atom_j, atom_k,
                                           atom_l, invalid_geometry_term);
        ...
        const double nphi = temp_pn * geometry.phi;
        const double cos_nphi = cos(nphi);
        const double sin_nphi = sin(nphi);
        const double energy_double =
            temp_pk + cos_nphi * temp_gamc + sin_nphi * temp_gams;
        const double derivative =
            temp_pn * (-temp_gamc * sin_nphi + temp_gams * cos_nphi);
        float energy = 0.0f;
        VECTOR fi;
        VECTOR fj;
        VECTOR fk;
        VECTOR fl;
        if (!Torsion_Double_Is_Finite(nphi) ||
            !Torsion_Double_Is_Finite(cos_nphi) ||
            !Torsion_Double_Is_Finite(sin_nphi) ||
        ...
        if (need_virial && atom_i < local_atom_numbers)
        {
            ...
            {
                Dihedral_Fail_Invalid_Geometry(global_term, atom_i, atom_j,
                                               atom_k, atom_l,
        ...
        {
            Dihedral_Fail_Invalid_Geometry(global_term, atom_i, atom_j, atom_k,
                                           atom_l, invalid_geometry_term, true);
            di_ene[dihedral_i] = 0.0f;
#ifdef USE_GPU
            return;
#else
...
void DIHEDRAL::Initial(CONTROLLER* controller, const char* module_name)
{
    ...
    if (torsions_to_use != NULL)
    {
        const std::string validation_error = Validate_Torsions(
            *torsions_to_use, Xponge::system.atoms.mass.size(), "proper",
            false);
        if (!validation_error.empty())
        {
            const std::string reason =
                "Reason:\n\tinvalid proper dihedral "
                "data: " +
                validation_error + "\n";
            controller->Throw_SPONGE_Error(spongeErrorBadFileFormat,
                                           "DIHEDRAL::Initial", reason.c_str());
        }
...
static __global__ void get_local_device(
    ...
    const int* d_ipn, const float* d_pk, const float* d_gamc,
    const float* d_gams, const float* d_pn, int* d_ipn_local, float* d_pk_local,
    float* d_gamc_local, float* d_gams_local, float* d_pn_local,
    int* d_global_index_local, int* d_num_dihe_local, int* d_invalid_local_term,
    int* d_invalid_local_atom)
{
    ...
    for (int i = 0; i < dihedral_numbers; i++)
    {
            ...
            atom_local_label[global_atoms[3]] == 1)
        {
            ...
            d_ipn_local[local_term] = d_ipn[i];
            d_pk_local[local_term] = d_pk[i];
            d_gamc_local[local_term] = d_gamc[i];
            d_gams_local[local_term] = d_gams[i];
            d_pn_local[local_term] = d_pn[i];
                         ...
                         int* atom_local_id)
{
        ...
        local_atom_numbers > std::numeric_limits<int>::max() - ghost_numbers)
    {
        controller->Throw_Formatted_SPONGE_Error(
            spongeErrorSimulationBreakDown, "DIHEDRAL::Get_Local",
    ...
    Launch_Device_Kernel(get_local_device, 1, 1, 0, NULL, dihedral_numbers,
                         ...
                         d_atom_d_local, d_ipn, d_pk, d_gamc, d_gams, d_pn,
                         d_ipn_local, d_pk_local, d_gamc_local, d_gams_local,
                         d_pn_local, d_global_index_local, d_num_dihe_local,
    ...
    if (invalid_term >= 0)
    {
        ...
        controller->Throw_Formatted_SPONGE_Error(
            spongeErrorSimulationBreakDown, "DIHEDRAL::Get_Local",
            "Reason:\n\t%s proper dihedral term %d (global atoms %d %d %d "
    ...
    LTMatrix3* atom_virial)
{
    if (is_initialized &&
        num_dihe_local >
            0)  // 修改：删除MPI_rank==0判断，求和变为局部求和，加入判断是否需要计算atom_energy和virial
    {
#ifndef GPU_ARCH_NAME
        ...
        Launch_Device_Kernel(
            ...
            this->d_pk_local, this->d_gamc_local, this->d_gams_local,
            this->d_pn_local, this->d_global_index_local, frc, need_atom_energy,
        ...
        if (invalid_code != -1)
        {
            ...
            controller->Throw_Formatted_SPONGE_Error(
                ...
                "Reason:\n\t%s proper dihedral term %d (global atoms %d %d "
                "%d %d) %s\n",
                module_name, invalid_term, h_atom_a[invalid_term],
                h_atom_b[invalid_term], h_atom_c[invalid_term],
...
void DIHEDRAL::Step_Print(CONTROLLER* controller, bool print_sum)
{
    if (is_initialized && CONTROLLER::MPI_rank < CONTROLLER::PP_MPI_size)
    {
        ...
        {
            controller->Throw_Formatted_SPONGE_Error(
                spongeErrorSimulationBreakDown, "DIHEDRAL::Step_Print",
                "Reason:\n\t%s proper dihedral total energy is non-finite "
