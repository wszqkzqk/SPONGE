                                ...
                                std::uint64_t seed)
{
    ...
    for (int atom = 0; atom < md_info->atom_numbers; ++atom)
    {
        ...
        if (!Float_Memory_Is_Finite(&md_info->h_mass[atom]) || mass < 0.0)
        {
            controller->Throw_Formatted_SPONGE_Error(
                spongeErrorValueErrorCommand, kErrorBy,
        ...
        VECTOR velocity = {static_cast<float>(sigma * normal(generator)),
                           ...
                           static_cast<float>(sigma * normal(generator))};
        if (!Float_Memory_Is_Finite(&velocity.x) ||
            !Float_Memory_Is_Finite(&velocity.y) ||
            !Float_Memory_Is_Finite(&velocity.z))
        {
    ...
    if (!Double_Memory_Is_Finite(&total_mass) || !(total_mass > 0.0))
    {
        controller->Throw_SPONGE_Error(
            spongeErrorValueErrorCommand, kErrorBy,
                            ...
                            const DOMAIN_INFORMATION* dd)
{
    ...
    for (int atom = 0; atom < dd->atom_numbers; ++atom)
    {
        ...
        if (!Double_Memory_Is_Finite(&contribution) || contribution < 0.0)
        {
            controller->Throw_SPONGE_Error(
    ...
    if (!Double_Memory_Is_Finite(&kinetic_energy) || kinetic_energy < 0.0)
    {
        controller->Throw_SPONGE_Error(
            spongeErrorSimulationBreakDown, kFinalizeErrorBy,
                                           ...
                                           MD_INFORMATION* md_info)
{
    ...
    if (!Float_Memory_Is_Finite(&md_info->sys.target_temperature) ||
                                            ...
                                            SETTLE* settle, SHAKE* shake)
{
    ...
    const double projected_temperature =
        ...
        (static_cast<double>(md_info->sys.freedom) *
         static_cast<double>(CONSTANT_kB));
    if (!Double_Memory_Is_Finite(&projected_temperature) ||
    ...
    if (!Double_Memory_Is_Finite(&target_kinetic_energy) ||
        !(target_kinetic_energy > 0.0) ||
        !Double_Memory_Is_Finite(&scale_double) || !(scale_double > 0.0) ||
        !Float_Memory_Is_Finite(&scale) || !(scale > 0.0f))
    {
        controller->Throw_SPONGE_Error(
            spongeErrorSimulationBreakDown, kFinalizeErrorBy,
    ...
    if (!Double_Memory_Is_Finite(&final_temperature) ||
        !Float_Memory_Is_Finite(&stored_temperature) ||
    ...
    const double relative_temperature_error =
        std::abs(final_temperature -
                 static_cast<double>(md_info->sys.target_temperature)) /
        static_cast<double>(md_info->sys.target_temperature);
    if (!Double_Memory_Is_Finite(&relative_temperature_error) ||
